...
export class AuthController {
  ...
  private setSessionCookie(res: Response, idToken: string) {
  ...
  try {
    ...
    if (this.wantsHTML(req)) return res.redirect(303, '/onboarding');
    return res.status(201).json(player);
  } catch (e: any) {
    const code = e?.code || e?.message || 'signup_error';
    console.error('[signup_error]', code, e);
    if (this.wantsHTML(req)) return res.redirect(303, `/register?error=signup&code=${encodeURIComponent(code)}`);
  ...
  login = async (req: Request, res: Response) => {
    try {
      ...
      if (this.wantsHTML(req)) return res.redirect(303, '/home'); // evita re-POST
      return res.status(200).json(session);
    } catch (e: any) {
      const code = e?.code || e?.message || 'invalid_credentials';
      console.error('[login_error]', code, e);
      if (this.wantsHTML(req)) return res.redirect(303, `/login?error=login&code=${encodeURIComponent(code)}`);
