...
function dedupe(arr: string[] | undefined): string[] | undefined {
  if (!arr) return undefined;
  return Array.from(new Set(arr.map((s) => String(s).trim()).filter(Boolean)));
}

function normPlatforms(arr: string[] | undefined): string[] | undefined {
  if (!arr) return undefined;
  const out = arr
...
export class PresenceService {
  ...
  async me(uid: string): Promise<PresenceRecord | null> {
  ...
  async setOnline(input: PresenceUpsertInput): Promise<void> {
    ...
    const payload: PresenceUpsertInput = {
      uid: input.uid,
  ...
  async listSimilarOnline(query: SimilarPlayersQuery): Promise<PresenceRecord[]> {
    if (!query?.uid) throw new Error('missing_uid');
    const normalized: SimilarPlayersQuery = {
      uid: query.uid,
